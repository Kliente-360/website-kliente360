...
The problem isn't scarcity of choice — it's the opposite. With six paths and a mutual-exclusion rule buried in the middle of them, the question that decides the budget isn't "how much does Agentforce cost," it's "which of these six models matches the usage pattern the organization already has." That budget question arrives even before the bigger one — [whether that pilot will actually reach production](/blog/en/piloto-agentforce-producao.html), a stage fewer than 10% of Salesforce's own customers complete today.
...
2. **$2-per-conversation.** Built for agents facing external customers. A conversation is the interaction window between user and agent — from the first message to resolution, escalation to a human, or 24 hours of inactivity — billed as one closed unit, regardless of how many messages or actions happened inside it. When the channel is WhatsApp, starting October 2026 this cost stacks with [the per-service-message fee Meta itself starts charging](/blog/en/whatsapp-salesforce-brasil.html) — two layers of variable cost on the same conversation.
