...
2. **Model choice by use case, not global.** Using GPT-4o or Claude Sonnet to classify intent is waste. For complex response generation, worth it. For short summarization, a mid-tier model (Haiku, GPT-4o mini) delivers 90% of the quality at 10% of the price. Systems using the same model for everything pay 5–10× more than systems with routing. Taken to the extreme, that routing is [SLM as the default in an agent pipeline](/blog/en/slm-pipeline-de-agentes.html): reserve the frontier model for tasks that need generalization and use a small model — 10 to 30x cheaper — for everything repetitive.
