...
3. **Source freshness como test.** Definir `freshness` en cada `source` (Salesforce, Stripe, Segment, etc.) con SLA explícito. Cuando la fuente atrasa, el equipo se entera antes de que el dashboard salga mal. Es uno de los tests más subvalorados de dbt — la base más simple de los [cinco ejes de observabilidad de datos](/blog/es/observabilidade-de-dados.html): el frescor detecta la mayor parte de las fallas de pipeline con el menor esfuerzo de instrumentación.
