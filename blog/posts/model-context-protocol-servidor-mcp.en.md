...
2. **Governance on the right side.** Exposing via MCP moves access control inside the server — who can call which tool, with what scope. If that control doesn't exist even in the current API, layering MCP on top doesn't solve the governance problem; it just moves where it shows up. That formal control also doesn't substitute for actively testing the vectors [most agent pilots never simulate](/blog/en/seguranca-de-agentes-piloto-nao-testa.html), like prompt injection and tool poisoning.
