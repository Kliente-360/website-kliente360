...
2. **Gobernanza del lado correcto.** Exponer vía MCP mueve el control de acceso hacia adentro del servidor —quién puede llamar qué tool, con qué alcance. Si ese control no existe ni en la API actual, agregar MCP encima no resuelve el problema de gobernanza; solo cambia dónde aparece. Ese control formal tampoco reemplaza probar activamente los vectores que [la mayoría de los pilotos de agente nunca simula](/blog/es/seguranca-de-agentes-piloto-nao-testa.html), como prompt injection y tool poisoning.
...
Esa portabilidad recién ganada en la conexión entre agente y herramienta no elimina el aprisionamiento —solo [desplaza el lock-in a otro piso del harness](/blog/es/lock-in-plataforma-de-agentes.html), la orquestación y la memoria de contexto, que siguen siendo propietarias por diseño en toda plataforma de agente.
