...
2. **Is the use case simple replication or heavy transformation?** Zero-ETL handles "bring transactional data into the warehouse near real-time" well — [the same freshness an autonomous agent demands](/blog/en/dado-pronto-para-ia-arquitetura-agente.html) from the data it consumes. It doesn't handle complex joins across multiple sources, deduplication, or business rules that need to run before load — that still requires dbt, testing and a defined owner, just executed inside the destination warehouse now.
