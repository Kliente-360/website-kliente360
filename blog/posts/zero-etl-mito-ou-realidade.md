...
2. **O caso de uso é replicação simples ou transformação pesada?** Zero-ETL resolve bem "trazer dado transacional pro warehouse quase em tempo real". Não resolve join complexo entre múltiplas fontes, deduplicação, nem regra de negócio que precisa rodar antes da carga — isso continua exigindo dbt, teste e dono definido, só que executado dentro do warehouse de destino.
